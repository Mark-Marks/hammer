...
- Integrated IsA with refs. It's now possible to pass a jecs id which'll be added to an entity created by ref (`ref(..., id)`). This is meant to be used with the IsA addon, in order to make sure entities adhere to a certain structure with pre-initialized data and it's safe to use them.

### Changed

- `@native` everywhere, perhaps even in your closet
